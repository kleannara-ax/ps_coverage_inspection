...
COUNT=$(echo "$RESP" | python3 -c "import sys,json; print(json.load(sys.stdin)['totalElements'])")
...
FIRST_ID=$(echo "$RESP" | python3 -c "import sys,json; print(json.load(sys.stdin)['content'][0]['id'])")
HTTP_CODE=$(curl -s -o /tmp/uc3c.json -w '%{http_code}' "$API/$FIRST_ID")
assert_eq "UC3-C 개별 레코드 GET /id → HTTP 200" "200" "$HTTP_CODE"
SINGLE_INDBCD=$(cat /tmp/uc3c.json | python3 -c "import sys,json; print(json.load(sys.stdin)['indBcd'])")
...
ACTION1=$(echo "$RESP1" | python3 -c "import sys,json; print(json.load(sys.stdin)['_action'])")
SEQ1=$(echo "$RESP1" | python3 -c "import sys,json; print(json.load(sys.stdin)['seq'])")
BCDSEQ1=$(echo "$RESP1" | python3 -c "import sys,json; print(json.load(sys.stdin)['indBcdSeq'])")
...
BCDSEQ2=$(echo "$RESP2" | python3 -c "import sys,json; print(json.load(sys.stdin)['indBcdSeq'])")
assert_eq "UC4-B 3차 재저장 → indBcdSeq=3" "3" "$BCDSEQ2"

# 4-C: UPDATE 후 총 건수 변동 없음 (3건 유지)
RESP3=$(curl -s "$API?page=0&size=20")
COUNT3=$(echo "$RESP3" | python3 -c "import sys,json; print(json.load(sys.stdin)['totalElements'])")
...
EXISTS=$(echo "$CHKR" | python3 -c "import sys,json; print(json.load(sys.stdin)['exists'])")
assert_eq "UC4-E check-exists → True" "True" "$EXISTS"
CHKR_NONE=$(curl -s "${API}/check-exists?matnr=NONE&lotnr=NONE&indBcd=NONE")
EXISTS_NONE=$(echo "$CHKR_NONE" | python3 -c "import sys,json; print(json.load(sys.stdin)['exists'])")
...
HITS=$(echo "$RESP" | python3 -c "import sys,json; print(json.load(sys.stdin)['totalElements'])")
...
RESP=$(curl -s "$API?page=0&size=20")
PAGE0_COUNT=$(echo "$RESP" | python3 -c "import sys,json; d=json.load(sys.stdin); print(len(d['content']))")
TOTAL_PAGES=$(echo "$RESP" | python3 -c "import sys,json; print(json.load(sys.stdin)['totalPages'])")
TOTAL_EL=$(echo "$RESP" | python3 -c "import sys,json; print(json.load(sys.stdin)['totalElements'])")
...
PAGE1_COUNT=$(echo "$RESP" | python3 -c "import sys,json; d=json.load(sys.stdin); print(len(d['content']))")
assert_eq "UC7-B page=1 → 5건" "5" "$PAGE1_COUNT"

# 7-C: 큰 페이지 번호 → 0건
RESP=$(curl -s "$API?page=99&size=20")
PAGE99_COUNT=$(echo "$RESP" | python3 -c "import sys,json; d=json.load(sys.stdin); print(len(d['content']))")
assert_eq "UC7-C page=99 → 0건" "0" "$PAGE99_COUNT"

# 7-D: size=5 → totalPages=5
RESP=$(curl -s "$API?page=0&size=5")
TP5=$(echo "$RESP" | python3 -c "import sys,json; print(json.load(sys.stdin)['totalPages'])")
assert_eq "UC7-D size=5 → totalPages=5" "5" "$TP5"

# 7-E: first/last 플래그 확인
IS_FIRST=$(echo "$RESP" | python3 -c "import sys,json; print(json.load(sys.stdin)['first'])")
IS_LAST=$(echo "$RESP" | python3 -c "import sys,json; print(json.load(sys.stdin)['last'])")
...
IS_LAST2=$(echo "$RESP_LAST" | python3 -c "import sys,json; print(json.load(sys.stdin)['last'])")
assert_eq "UC7-E 마지막 페이지 → last=True" "True" "$IS_LAST2"

# 7-F: size 제한 (max 100)
RESP=$(curl -s "$API?page=0&size=999")
RETURNED=$(echo "$RESP" | python3 -c "import sys,json; d=json.load(sys.stdin); print(len(d['content']))")
...
# 현재 건수 확인
BEFORE=$(curl -s "$API?page=0&size=100" | python3 -c "import sys,json; print(json.load(sys.stdin)['totalElements'])")
echo -e "  삭제 전 건수: $BEFORE"

# 9-A: 특정 레코드 삭제
FIRST_ID=$(curl -s "$API?page=0&size=1" | python3 -c "import sys,json; print(json.load(sys.stdin)['content'][0]['id'])")
HTTP_CODE=$(curl -s -o /dev/null -w '%{http_code}' -X DELETE "$API/$FIRST_ID")
assert_eq "UC9-A 개별 삭제 → HTTP 200" "200" "$HTTP_CODE"

# 9-B: 삭제 후 건수 -1
AFTER=$(curl -s "$API?page=0&size=100" | python3 -c "import sys,json; print(json.load(sys.stdin)['totalElements'])")
...
AFTER_ALL=$(curl -s "$API?page=0&size=20" | python3 -c "import sys,json; print(json.load(sys.stdin)['totalElements'])")
...
STORED_BCD=$(cat /tmp/uc10a.json | python3 -c "import sys,json; print(json.load(sys.stdin)['indBcd'])")
...
STORED_LEN=$(cat /tmp/uc10c.json | python3 -c "import sys,json; print(len(json.load(sys.stdin)['indBcd']))")
...
UNICODE_BCD=$(cat /tmp/uc10e.json | python3 -c "import sys,json; print(json.load(sys.stdin)['indBcd'])")
...
BULK_COUNT=$(curl -s "$API?page=0&size=100" | python3 -c "import sys,json; print(json.load(sys.stdin)['totalElements'])")
